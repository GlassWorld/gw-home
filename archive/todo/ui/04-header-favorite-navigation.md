...
- [x] 사이드메뉴에서 메뉴별 즐겨찾기 체크/해제 가능
- [x] 즐겨찾기 설정은 사용자별로 독립 저장
- [x] 헤더에 개인 즐겨찾기 메뉴를 노출
- [x] 관리자 메뉴는 권한에 맞게만 저장/노출

---

## Step 1. DB — profile 에 즐겨찾기 설정 컬럼 추가

- [x] `tb_mbr_prfl` 에 즐겨찾기 메뉴 저장 컬럼 추가
  - 후보: `fav_menu_json TEXT`
- [x] nullable 허용, 미설정 시 빈 배열로 해석
- [x] rollback 또는 참고 DDL 주석 포함
...
- [x] `PrflVo` 에 즐겨찾기 메뉴 저장 필드 추가
- [x] 기존 공개 프로필 응답에는 노출하지 않도록 범위 유지
...
- [x] profile mapper 에 즐겨찾기 조회 메서드 추가
- [x] profile mapper 에 즐겨찾기 저장 메서드 추가
- [x] account idx 기준 조회/저장 처리
...
- [x] 본인 계정 기준 즐겨찾기 조회 로직 추가
- [x] 저장 시 허용 메뉴 화이트리스트 검증
- [x] 관리자 전용 메뉴 경로 저장 권한 검증
- [x] 잘못된 경로/중복/개수 초과 처리
...
- [x] `GET /api/v1/profiles/me/navigation-favorites`
- [x] `PUT /api/v1/profiles/me/navigation-favorites`
...
- [x] 헤더/사이드가 공통으로 쓰는 메뉴 정의 작성
- [x] 일반 메뉴 / 관리자 메뉴 / 즐겨찾기 가능 여부 메타 포함
- [x] route path 와 label 을 한 소스에서 관리
...
- [x] 즐겨찾기 API 타입 정의 추가
- [x] 조회/저장 composable 추가
- [x] 헤더/사이드가 함께 사용할 상태 관리 추가
...
- [x] 각 메뉴 항목 우측에 별표 토글 추가
- [x] 별표 클릭 시 이동 이벤트와 분리
- [x] 저장 중 상태 또는 즉시 반영 UX 결정
- [x] 관리자 메뉴는 권한 있을 때만 즐겨찾기 가능
...
- [x] 로그인 후 즐겨찾기 설정 초기 로딩
- [x] 새로고침 시 헤더/사이드 상태 유지
- [x] 로그아웃 시 즐겨찾기 상태 초기화
...
- [x] 저장 포맷을 `TEXT JSON 문자열`로 갈지, 별도 테이블로 갈지
