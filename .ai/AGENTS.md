...
- **키 관리 원칙** — 내부 식별자는 `id`, 외부 식별자는 `uuid` 사용
- **API 식별자 원칙** — API 요청/응답의 식별자는 `uuid` 전용, 내부 DB PK(`_idx`) 직접 노출 금지
...
- DB 컬럼이 `{table}_idx`, `{table}_uuid` 여도 내부 코드에서는 `id`, `uuid` 기준으로 매핑
...
운영 규칙:
- 두 디렉토리에는 동일한 스킬 파일이 모두 존재해야 한다.
- 한쪽 스킬을 수정하면 다른 쪽도 동일하게 맞춘다.
...
- 진행 중 문서는 `work/review/`, `work/todo/` 아래에 둔다.
- 완료 기준:
  - `todo` 문서에 미체크 항목(`- [ ]`)이 없음
  - 해당 작업이 실제로 마무리되어 더 이상 활성 작업 큐에 두지 않아도 됨
- 완료된 문서는 자동 이동하지 않는다. 관련 문서 정리는 `##커밋` 진행 시 처리한다.

충돌 시 우선순위: 시스템/개발자 지침 → `.ai/CORE_RULES.md` → 작업 관련 문서 → `.ai/skill/*`, `.claude/skill/*` 순서
...
| 트리거 | 동작 |
|--------|------|
| `##검토` | `work/review/{domain}/review-{subject}.md` 생성/업데이트 → "계획 생성할까요?" 질문 |
| `##계획` | `work/todo/{domain}/todo-{subject}.md` 생성/업데이트 |
| `##작업` | 아래 작업 흐름 실행 |
| `##커밋` | 아래 커밋 흐름 실행 |

> `##검토`, `##계획` 요청 시 코드 변경 금지. `##작업` 요청 시만 문서 기반으로 코드 수정.

---

### `##커밋` 흐름

1. `git diff --staged` 및 `git status` 로 변경 내역 파악
2. 변경 성격 판단:
   - 버그 수정 → `fix.`
   - 새 기능 추가 → `feat.`
   - 그 외 (리팩토링, 문서, 설정 등) → `refactor.` / `docs.` / `chore.`
3. 커밋 메시지 작성 규칙:
   - 접두어: `fix.` / `feat.` / `refactor.` / `docs.` / `chore.`
   - 내용: **한글**로 변경 사항 요약 (무엇을, 왜)
   - 예시: `feat. 계정 설정 페이지 UI 구현`, `fix. 로그인 세션 만료 처리 오류 수정`
4. 사용자에게 커밋 메시지 초안 제시 → 승인 후 커밋 실행
5. `##커밋` 대상 작업과 직접 관련된 `work/review/`, `work/todo/` 문서는 완료된 것으로 간주하고 함께 삭제한다.
   - 커밋 전 관련 문서가 남아 있으면 삭제 대상으로 포함한다.
   - 관련 없는 문서는 삭제하지 않는다.
...
- `work/review/{domain}/review-{subject}.md` 문서를 먼저 생성 또는 업데이트
...
- 작업은 todo 항목 단위로 단계별 진행한다
- 각 항목 완료 즉시 todo 문서의 해당 항목을 체크 처리한다
  ```
  - [ ] 미완료
  - [x] 완료
  ```
- 단계 완료 후 다음 항목으로 진행한다 — 일괄 완료 후 한꺼번에 체크 금지
...
- **중복 생성 금지**
- **문서 연결 유지**: `work/review` → `work/todo` 흐름. 각 문서에 관련 문서 참조 포함.
...
```
work/review/{domain}/review-{subject}.md

예시:
  work/review/auth/review-init-admin.md
  work/review/board/review-board-list-api.md
  work/review/account/review-signup-flow.md
```

- `{domain}`: 작업 대상 도메인 (auth, account, board, admin, file, profile 등)
- `{subject}`: 작업 주제를 kebab-case로 요약

#### todo 문서

```
work/todo/{domain}/todo-{subject}.md

예시:
  work/todo/auth/todo-init-admin.md
  work/todo/board/todo-board-search.md
  work/todo/account/todo-signup-flow.md
```
