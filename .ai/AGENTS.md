...
- 진행 중 문서는 루트 `review/`, `todo/` 아래에 둔다.
- **완료된 작업 문서**는 루트 `archive/` 아래로 이동한다.
  - `archive/review/...`
  - `archive/todo/...`
- 완료 기준:
  - `todo` 문서에 미체크 항목(`- [ ]`)이 없음
  - 해당 작업이 실제로 마무리되어 더 이상 활성 작업 큐에 두지 않아도 됨
- 문서 이동 시 가능한 한 기존 하위 디렉토리 구조를 유지한다.
...
| 트리거 | 동작 |
|--------|------|
| `##검토` | review 문서 생성/업데이트 → "계획 생성할까요?" 질문 |
| `##계획` | todo 문서 생성/업데이트 |
| `##작업` | 아래 작업 흐름 실행 |
| `##커밋` | 아래 커밋 흐름 실행 |
...
```
review/{domain}/{number}-{filename}.md

예시:
  review/auth/01-init-admin.md
  review/board/01-board-list-api.md
  review/account/01-signup-flow.md
```

- `{domain}`: 작업 대상 도메인 (auth, account, board, admin, file, profile 등)
- `{number}`: 도메인 내 순번 (01, 02, ...)
- `{filename}`: 작업 주제를 kebab-case로 요약

#### todo 문서

```
todo/{domain}/{number}-{filename}.md

예시:
  todo/auth/01-init-admin.md
  todo/board/01-board-search.md
  todo/account/01-signup-flow.md
```
