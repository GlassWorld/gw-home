...
- [x] 카테고리 필터를 복수 선택 토글 구조로 확장
- [x] URL query 와 초기 진입 상태에서 복수 카테고리 선택 복원

### 4. Category Filter — Backend / Query

- [x] 목록 조회 API 가 복수 카테고리 파라미터를 수용하도록 확장
- [x] Mapper 에서 선택된 카테고리 중 하나라도 매칭되는 조건으로 조정
- [x] 기존 단일 `categoryUuid` 링크도 호환되도록 유지
...
- [x] 카테고리 2개 이상 선택 시 복수 조건으로 목록이 조회되는지 확인
- [x] 전체 해제 시 전체 목록으로 복귀하는지 확인
- [x] 기존 단일 카테고리 URL 로 진입해도 정상 동작하는지 확인
